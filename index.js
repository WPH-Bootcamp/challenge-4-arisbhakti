 ...
 * Menggabungkan timestamp dan random string agar unik
 */
...
function markTodoCompleted() {
  if (todos.length === 0) {
    console.log("No to-dos available to mark as completed.");
    return;
  }
  ...
  if (!input || Number.isNaN(index) || index < 1 || index > todos.length) {
...
/**
 * Menampilkan menu command
 */
function showMenu() {
  console.log("\n=== TO-DO APP MENU ===");
  console.log("1 or 'add'      - Add a new to-do");
  console.log("2 or 'complete' - Mark a to-do as completed");
  console.log("3 or 'delete'   - Delete a to-do");
  console.log("4 or 'list'     - List all to-dos");
  console.log("5 or 'exit'     - Exit the app");
}

/**
 * Main loop aplikasi to-do
 */
function runTodoApp() {
  let running = true;
  ...
  while (running) {
    showMenu();
    const rawInput = prompt("Enter a command: ") || "";
    let command = rawInput.trim().toLowerCase();

    // Mapping numeric commands ke text
    switch (command) {
      case "1":
        command = "add";
        break;
